...
def validate_identifier_token(token: str, *, kind: str) -> str:
    token = token.strip()
    if not _IDENTIFIER_RE.fullmatch(token):
        raise RuntimeError(f"invalid Iceberg {kind} identifier token: {token!r}")
    return token


def sanitize_table_id(table_id: str) -> str:
    qualified = qualify_table_id(table_id)
    catalog_name, namespace, table_name = parse_table_id(qualified)
    catalog_name = validate_identifier_token(catalog_name, kind="catalog")
    namespace = validate_identifier_token(namespace, kind="namespace")
    table_name = validate_identifier_token(table_name, kind="table")
...
def parse_table_list(env_name: str, default_value: str) -> list[str]:
    ...
    if not raw:
        return []
    tables = [sanitize_table_id(item) for item in split_csv(raw)]
...
def rewrite_data_files_call(table_id: str, where_clause: str) -> str:
    ...
    return (
        ...
        f"options => map("
        f"'min-input-files', '2', "
        f"'remove-dangling-deletes', 'true'))"
    )
...
def maintenance_optimize() -> MaintenanceResult:
    ...
    for table_id in rewrite_tables:
        ...
        if not table_has_column(spark, table_id, "as_of_date"):
            skipped.append(table_id)
            table_results.append(
                table_op_result(
                    table_id=table_id,
