...
# =============================================================================
# Iceberg REST Catalog Validation Suite
# Usage: ./iceberg_server_validate.sh [--validate] | --help
# =============================================================================
...
SPARK_PROBE_IMAGE="${SPARK_PROBE_IMAGE:-${ELT_TASK_IMAGE:-ghcr.io/athithya-sakthivel/flyte-elt-task}}"
# --- Logging ------------------------------------------------------------------
...
validate_server_spec() {
  ...
  # FIX: Use explicit cleanup instead of trap with scoped variables
  cleanup() {
    rm -f "${dep_json:-}" "${secret_json:-}" "${svc_json:-}" "${eps_json:-}"
  }
  trap cleanup RETURN

  log "validating server specification"
...
PY
  ...
  log "server specification validated"
}
...
validate() {
  ...
  log "validating iceberg deployment"
...
Options:
  --validate    Run full validation suite (default if no args)
  --help, -h    Show this help message

Examples:
  $0 --validate    # Run validation
  $0               # Run validation (default)
EOF
}
